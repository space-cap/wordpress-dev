...
<nav id="gnb" class="fixed top-0 left-0 right-0 z-50 w-full transition-all duration-300 bg-transparent border-b border-white/10">
            ...
            <ul class="flex items-center gap-[30px] xl:gap-[40px]">
                ...
                <li class="relative group">
                    <button class="gnb-menu-link text-[18px] font-medium text-white hover:opacity-75 transition-all duration-300 flex items-center gap-1.5 focus:outline-none">
                        이엘 피드
                        <svg class="w-3.5 h-3.5 transition-transform duration-300 group-hover:rotate-180" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M19 9l-7 7-7-7"></path></svg>
                    </button>
                ...
                <!-- 테마 토글 버튼 (데스크톱) -->
                <li>
                    <button id="theme-toggle-btn" class="gnb-menu-link flex items-center justify-center w-10 h-10 rounded-full border border-white/20 hover:bg-white/10 text-white transition-all duration-300 focus:outline-none" aria-label="테마 전환">
                        <!-- 해 아이콘 (다크모드 시 노출) -->
                        <svg id="theme-icon-sun" class="w-5 h-5 hidden" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364-6.364l-.707.707M6.343 17.657l-.707.707m12.728 0l-.707-.707M6.343 6.344l-.707-.707M12 8a4 4 0 100 8 4 4 0 000-8z"></path></svg>
                        <!-- 달 아이콘 (라이트모드 시 노출) -->
    ...
    <!-- 모바일 메뉴 오버레이 -->
    <div id="mobile-menu" class="hidden lg:hidden fixed inset-0 bg-[#1a1a1a] dark:bg-[#0b0f19] z-50 flex flex-col justify-between p-6 overflow-y-auto">
        <div class="flex justify-between items-center h-[60px] shrink-0">
            ...
            <div class="flex items-center gap-4">
                <!-- 테마 토글 버튼 (모바일) -->
                <button id="mobile-theme-toggle-btn" class="flex items-center justify-center w-10 h-10 rounded-full border border-white/20 hover:bg-white/10 text-white transition-all duration-300 focus:outline-none" aria-label="테마 전환">
                    <!-- 해 아이콘 -->
                    <svg id="mobile-theme-icon-sun" class="w-5 h-5 hidden" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364-6.364l-.707.707M6.343 17.657l-.707.707m12.728 0l-.707-.707M6.343 6.344l-.707-.707M12 8a4 4 0 100 8 4 4 0 000-8z"></path></svg>
                    <!-- 달 아이콘 -->
                    <svg id="mobile-theme-icon-moon" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z"></path></svg>
                </button>
                <button id="mobile-menu-close" class="p-2" aria-label="메뉴 닫기">
                    <span class="block w-[28px] h-[1.5px] bg-white rotate-45 translate-y-[0.75px] transition-all duration-300"></span>
                    <span class="block w-[28px] h-[1.5px] bg-white -rotate-45 -translate-y-[0.75px] transition-all duration-300"></span>
                </button>
            </div>
        </div>
